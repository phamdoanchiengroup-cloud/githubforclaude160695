...
- **Bố cục chuyên sâu thống nhất:** tab Tử Vi chia Phần 1–4 (`khoiPhan`, chân dung lá số gộp một khung; thẻ xem nhanh cung chỉ hiện khi bấm). Tab Hà Lạc tự dựng Phần 1–5 trong `renderHaLac` (`PHAN_HE.halac = null`). Chiêm tinh/Thần số/HD được `xepPhan` (theo `PHAN_HE`) xếp vào Phần 1–4 trước khi gom vào `details.chuyen-sau`.
...
- Chủ dự án muốn làm **kỹ từng hệ một**. Đã xong: Bát Tự 9 bước; Hà Lạc lục hào (HaLac.gs: `hlLucHao_` nạp giáp/Thế Ứng/lục thân/lục thú/phục thần/vượng suy theo tháng–ngày sinh/không vong/hào động hóa; `hlLinhVuc_` 8 lĩnh vực; `hlThoiVi_` hóa công – nguyên khí; `hlQueKhac_` Biến/Thác/Tổng). Đang làm: **Thần số học** theo quy trình 6 bước chủ dự án gửi (chỉ số cốt lõi + bổ sung, chu kỳ, luận tổng hợp 5.1–5.5, lời khuyên, trường hợp đặc biệt, biểu đồ). Sau đó Chiêm tinh, Human Design.
